...
void setup_sunspec_models() {
    ...
    set_sunspec_string(&holdingRegisters[common_offset + COMMON_MODEL], "SALV", 32);
    set_sunspec_string(&holdingRegisters[common_offset + COMMON_OPTIONS], "EMS", 16);
    ...
    //set_sunspec_string(&holdingRegisters[common_offset + COMMON_DEVICE_ADDR], "1", 16);
...
void update_sunspec_from_solark() {
    ...
    holdingRegisters[inverter_offset] = 0xFFFF;
    holdingRegisters[inverter_offset + 1] = 0x0000;
}
